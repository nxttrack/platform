...
- `tenant-documents`, `diploma-vault`, `participant-media` and `badge-studio-assets` therefore require their own versioned off-platform object copy plus an inventory that can be reconciled with `storage.objects`.
...
1. Recreate/verify the private `tenant-documents`, `diploma-vault`, `participant-media` and `badge-studio-assets` buckets and their policies through reviewed migrations/configuration.
