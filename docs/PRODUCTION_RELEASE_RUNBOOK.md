...
- Confirm an object-backup route for `tenant-documents`, `diploma-vault`, `participant-media` and `badge-studio-assets`; database backups contain Storage metadata, not the object bytes.
