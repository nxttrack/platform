...
- Confirm an object-backup route for `tenant-documents`, `diploma-vault` and `participant-media`; database backups contain Storage metadata, not the object bytes.
