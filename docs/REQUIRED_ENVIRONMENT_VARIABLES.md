...
| --- | --- | --- | --- | --- | --- | --- |
| `MAINTENANCE_NO_WRITE` | Platformbrede rollback-/onderhoudsblokkade voor onveilige HTTP-methoden | Alle applicatie-writes, server actions en interne/webhook-POST-routes | Aan tijdens certificatiepreview en rollback-containment | `false` | GitHub environment variable en VPS runtime-env | Alleen exact `true` blokkeert POST/PUT/PATCH/DELETE fail-closed met HTTP 503; Data API-toegang blijft een afzonderlijke boundary. |
...
| `NEWSLETTER_DELIVERY_ENABLED` | Tweede fail-closed capabilitygate voor nieuwsbriefdelivery | Nieuwsbriefplanning en -delivery | Uit totdat een sender geïmplementeerd en apart bewezen is | `false` | GitHub environment variable en VPS runtime-env | De huidige app houdt delivery ook bij `true` uit omdat er nog geen sender bestaat; de certificatiepreview forceert de waarde expliciet naar `false`. |
...
## Interne geplande jobs

| Naam | Doel | Module | Nu/later | Placeholder | Configureren | Opmerking |
| --- | --- | --- | --- | --- | --- | --- |
| `CRON_SECRET` | Beveiligt de interne Next Best Actions-, automation-recipe-, media-expiry-, zwemanalytics- en wekelijkse managementconcept-endpoints | Interne geplande jobs | Nodig voor geplande runs | `placeholder_add_later` | GitHub environment secret en VPS runtime-env | Gebruik minimaal 32 willekeurige tekens. Nooit committen. Managementsamenvattingen blijven altijd concept en worden nooit automatisch verstuurd. |
| `INTERNAL_JOBS_ENABLED` | Releasegate voor Next Best Actions, Automation Recipes, participant-media expiry, zwemanalytics/forecasts en wekelijkse managementconcepten | Geplande interne GitHub Actions-jobs | Na een bewezen deployment | `false` | GitHub environment variable per environment | Ontbrekend of iedere andere waarde dan exact `true` houdt deze jobs veilig uit. Zet staging pas aan nadat de matching SHA en migraties live zijn; productie uitsluitend binnen de goedgekeurde releaseprocedure. |


De automation-recipe runner, participant-media expiry, zwemanalyticsrunner en wekelijkse managementconcepten hergebruiken bewust hetzelfde interne `CRON_SECRET`; er is geen extra secret nodig. De zwemanalyticsjob reconcilieert alleen tenantgeïsoleerde lifecycle-events, snapshots, deterministische forecasts, accuracy en verlopen soft holds; hij verplaatst nooit een leerling. Tenantrecipes hebben daarnaast een database-afgedwongen `review_only`-grens en kunnen geen externe delivery activeren. Managementconcepten worden alleen als intern concept opgeslagen en nooit automatisch bezorgd.
