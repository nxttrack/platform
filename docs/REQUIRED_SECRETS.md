...
| `SEPA_CREDITOR_ID` | Creditor identifier for direct debit metadata if needed outside Mollie | Later | Bank/payment provider | Billing & Incasso Engine | `placeholder_add_later` |

Notes:

- Mollie may cover iDEAL and SEPA collection; direct bank/SEPA secrets should not be added unless the provider path requires it.
...
| Secret name | What it is for | Required now or later | Where to obtain it | Module | Safe placeholder |
| --- | --- | --- | --- | --- | --- |
| `WHATSAPP_BUSINESS_TOKEN` | WhatsApp Business API send/auth token | Later | Meta/WhatsApp Business platform | Communication Escalation Engine | `placeholder_add_later` |
| `WHATSAPP_BUSINESS_PHONE_NUMBER_ID` | WhatsApp sender phone number ID | Later | Meta/WhatsApp Business platform | Communication Escalation Engine | `placeholder_add_later` |
| `WHATSAPP_WEBHOOK_SECRET` | Verify inbound WhatsApp callbacks | Later | Generated during webhook setup | Communication Escalation Engine | `placeholder_add_later` |
...
| `CRM_TICKET_SYSTEM_API_KEY` | CRM/ticket provider integration | Later | Selected CRM/ticket provider | Tenant Helpdesk / Integration Layer | `placeholder_add_later` |
...
| SEPA incasso | `MOLLIE_API_KEY`, `MOLLIE_WEBHOOK_SECRET`, possibly `SEPA_CREDITOR_ID` |
...
| CRM/ticket integration | `CRM_TICKET_SYSTEM_API_KEY` |
