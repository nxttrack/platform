...
export type ParentPlanningPageProps = { searchParams?: Promise<ParentPortalSearchParams> };

export const dynamic = "force-dynamic";

export default async function ParentPlanningPage({ searchParams }: ParentPlanningPageProps) {
  ...
  return (
    <div className="space-y-6">
      <PageHeader kicker={`${titleCase(terminology.activities)}, ${terminology.makeUpActivities} en ${terminology.finalMoment}`} title="Planning" subtitle={`Alles rond de planning op één plek. Annuleren kan tot ${data.settings.lesson_cancellation_cutoff_hours} uur vooraf met automatische credit.`} />
      <Feedback saved={saved} error={error} terminology={terminology} />
      <ParentSectionNav
        items={[
          { active: true, href: participantContextHref("/portaal/planning#lessen", selectedParticipantId), label: titleCase(terminology.activities) },
          { href: participantContextHref("/portaal/planning#inhalen", selectedParticipantId), label: titleCase(terminology.makeUpActivities) },
          { href: participantContextHref("/portaal/planning#afzwemmen", selectedParticipantId), label: titleCase(terminology.finalMoment) },
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Metric icon={<Clock className="h-5 w-5" />} label={`Komende ${terminology.activities}`} value={lessonItems.filter((item) => new Date(item.session.starts_at).getTime() >= Date.now()).length} />
        <Metric icon={<RefreshCcw className="h-5 w-5" />} label="Beschikbare credits" value={visibleCredits.filter((credit) => credit.status === "available").length} />
        <Metric icon={<GraduationCap className="h-5 w-5" />} label={`Uitnodigingen ${terminology.finalMoment}`} value={visibleGraduationInvites.filter((invite) => invite.invite_status === "sent").length} />
      </div>
              ...
              return (
                <article className="rounded-xl border border-border bg-background p-4" key={offering.id}>
                  ...
                  <div className="mt-4 grid gap-2 sm:grid-cols-2">
                    <PlanningDetail icon={<CalendarCheck className="size-4" />} label={titleCase(terminology.activities)} value={`${occurrences.length} ${terminology.activities}`} />
                    <PlanningDetail icon={<CreditCard className="size-4" />} label="Betaling" value={paymentModeLabel(offering.payment_mode)} />
                    {occurrences[0] ? <PlanningDetail icon={<Clock className="size-4" />} label={`Eerste ${terminology.activity}`} value={formatDateTime(occurrences[0].starts_at)} /> : null}
      ...
      <section className="order-2 scroll-mt-24 rounded-xl border border-border bg-card p-5 shadow-soft" id="inhalen">
        <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
          <div>
            <h2 className="text-lg font-bold text-foreground">{titleCase(terminology.makeUpActivity)} kiezen</h2>
            <p className="mt-1 text-sm text-muted-foreground">Zet een beschikbare credit om naar een concreet passend moment.</p>
          </div>
          <StatusPill tone={catchUpData.options.length > 0 ? "success" : "neutral"}>{catchUpData.options.length} opties</StatusPill>
        </div>
        {visibleCredits.length === 0 ? (
          <EmptyState>Je hebt nog geen beschikbare credits voor {terminology.makeUpActivities}.</EmptyState>
        ) : (
                ...
                <article className="rounded-lg border border-border bg-white p-4" key={credit.id}>
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      ...
                      <h3 className="mt-1 font-bold text-foreground">Credit voor {terminology.makeUpActivity}</h3>
                  ...
                  {!canMutate ? (
                    ...
                    <p className="mt-3 text-sm text-muted-foreground">Geen passend moment met vrije capaciteit binnen de boekingsperiode.</p>
                  ) : (
                      ...
                      {options.map((option) => (
                        <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-border bg-muted/30 px-3 py-3" key={`${credit.id}:${option.sessionId}`}>
                          ...
                          <ConfirmActionForm
                            ...
                            confirmLabel={`${titleCase(terminology.makeUpActivity)} bevestigen`}
                            description={`De geldigheid, het niveau en de actuele vrije plek worden bij bevestiging opnieuw transactioneel gecontroleerd. Afhankelijk van de ${terminology.organization} volgt directe boeking of adminbeoordeling.`}
                            ...
                            title={`Deze ${terminology.makeUpActivity} kiezen?`}
      ...
      <section className="order-3 scroll-mt-24 rounded-xl border border-border bg-card p-5 shadow-soft" id="afzwemmen">
        <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
          <div>
            <h2 className="text-lg font-bold text-foreground">{titleCase(terminology.finalMoment)}</h2>
        ...
        {visibleGraduationInvites.length === 0 ? (
          <EmptyState>Er zijn nog geen uitnodigingen voor het {terminology.finalMoment}. Zodra een kind klaar is, verschijnt de uitnodiging hier.</EmptyState>
        ) : (
          <div className="grid gap-3 xl:grid-cols-2">
              ...
              return (
                <article className="rounded-xl border border-border bg-background p-4" key={invite.id}>
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      <p className="text-xs font-semibold uppercase tracking-wider text-primary">{participant?.display_name ?? "Kind"}</p>
                      <h3 className="mt-1 text-lg font-bold text-foreground">{event?.title ?? `Moment voor ${terminology.finalMoment}`}</h3>
                    </div>
      ...
      <div className="flex items-end justify-between gap-3">
        <div>
          <h2 className="text-lg font-bold text-foreground">{titleCase(terminology.activities)}</h2>
          <p className="mt-1 text-sm text-muted-foreground">Je actuele planning en details.</p>
        </div>
      ...
      {lessonItems.length === 0 ? (
        <EmptyState>Er zijn nog geen {terminology.activities} gekoppeld aan dit portaal.</EmptyState>
      ) : (
        <div className="space-y-3">
              ...
              <article className="rounded-xl border border-border bg-card p-4 shadow-soft" key={`${session.id}:${membership.participant_id}`}>
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-muted-foreground">{group?.name ?? "Groep"}</p>
                  </div>
                ...
                <div className="mt-4 flex flex-wrap items-end justify-between gap-3">
                  ...
                  {future && session.status === "scheduled" && !cancellation && canMutate ? <CancelForm participantId={membership.participant_id} session={session} onTime={onTime} terminology={terminology} /> : null}
...
function CancelForm({ onTime, participantId, session, terminology }: { onTime: boolean; participantId: string; session: SessionRow; terminology: PortalTerminology }) {
  return (
    <ConfirmActionForm
...
function Feedback({ saved, error, terminology }: { saved?: string; error?: string; terminology: PortalTerminology }) {
  ...
  if (saved === "confirmed") {
    return <p className="rounded-lg border border-success/20 bg-success/10 px-3 py-2 text-sm font-semibold text-success">Uitnodiging voor {terminology.finalMoment} bevestigd.</p>;
  }

  if (saved === "declined") {
    return <p className="rounded-lg border border-warning/20 bg-warning/10 px-3 py-2 text-sm font-semibold text-warning">Uitnodiging voor {terminology.finalMoment} afgewezen.</p>;
  }
  ...
  if (saved === "catchup-requested") {
    return <p className="rounded-lg border border-success/20 bg-success/10 px-3 py-2 text-sm font-semibold text-success">{titleCase(terminology.makeUpActivity)} aangevraagd. De administratie beoordeelt de aanvraag.</p>;
  }

  if (saved === "catchup-approved") {
    return <p className="rounded-lg border border-success/20 bg-success/10 px-3 py-2 text-sm font-semibold text-success">{titleCase(terminology.makeUpActivity)} ingepland.</p>;
  }
  ...
  if (error) {
    const message = error === "offering-full"
      ...
      : error.startsWith("offering")
        ? `De cursusplaats kon niet veilig worden gereserveerd. Probeer opnieuw of neem contact op met de ${terminology.organization}.`
