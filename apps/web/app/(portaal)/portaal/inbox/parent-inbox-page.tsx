  ...
  return (
    <div className="space-y-6">
      <PageHeader
        kicker="Communicatie"
        title="Inbox"
        subtitle={`Persoonlijke gesprekken, meldingen en belangrijke updates van de ${terminology.organization}.`}
      />
      ...
      <section className="scroll-mt-24 rounded-xl border border-border bg-card p-5 shadow-soft" id="meldingen">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">Voortgang, badges, documenten, betalingen en updates over het {terminology.finalMoment}.</p>
          </div>
          ...
          <div className="grid gap-3 lg:grid-cols-2">
            {visibleNotifications.map((notification) => (
              <article className="rounded-lg border border-border bg-white p-4" key={notification.id}>
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <StatusPill tone={notification.status === "unread" ? "info" : "neutral"}>{notificationLabel(notification.type, terminology.finalMoment)}</StatusPill>
...
function notificationLabel(type: string, finalMoment: string) {
