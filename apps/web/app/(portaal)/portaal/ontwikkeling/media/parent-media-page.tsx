...
export default async function ParentDevelopmentMediaPage({ searchParams }: PageProps) {
  const [data, params, portalTheme] = await Promise.all([
    getParentParticipantMediaData(),
    searchParams ?? Promise.resolve({}),
    resolveCurrentParentPortalTheme("/portaal/ontwikkeling/media")
  ]);
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        kicker="Ontwikkeling"
        subtitle={`Alleen jij en het bevoegde team van de ${terminology.organization} kunnen gepubliceerde momenten bekijken. Toestemming kan hier altijd worden ingetrokken.`}
        title="Besloten media"
      />
      <RouteFeedback
        success={saved ? consentSuccess(saved, terminology.organization) : null}
                ...
                <aside className="rounded-xl border border-border bg-muted/20 p-4">
                  ...
                  {consent?.status === "granted" && !overallConsent?.valid ? (
                    <p className="mt-3 rounded-lg border border-warning/20 bg-warning/10 p-3 text-xs leading-5 text-warning-foreground">
                      Jouw toestemming is actief, maar een keuze van een andere gekoppelde ouder/verzorger blokkeert publicatie. De {terminology.organization} ziet alleen de status, niet meer bewijs dan nodig.
                    </p>
                  ) : null}
