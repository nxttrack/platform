...
export default async function ParentBadgeWallPage({ searchParams }: { searchParams?: Promise<ParentPortalSearchParams> }) {
  ...
  return <div className="space-y-6">
    ...
    {visibleParticipants.map((participant) => {
      ...
      return <section className="space-y-4" key={participant.id}>
        <div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-primary">{titleCase(terminology.journey)}</p><h2 className="mt-1 text-2xl font-bold">{participant.display_name}</h2><p className="mt-1 text-sm text-muted-foreground">{participantAwards.length} mooie momenten vastgelegd</p></div><StatusPill tone="success">{participantAwards.length} behaald</StatusPill></div>
        ...
        <div className="space-y-5">
          {categories.map((category) => <section className="rounded-3xl border border-border bg-card/70 p-4 shadow-soft sm:p-5" key={category}>
            <div className="mb-4">
              ...
              <h3 className="mt-1 text-lg font-bold">{badgeCategoryLabel(category, terminology.finalCredentials, terminology.makeUpActivities, terminology.stage)}</h3>
            </div>
...
function badgeCategoryLabel(category: string, finalCredentials: string, makeUpActivities: string, stage: string) {
  return ({
    attendance: "Aanwezigheid",
    compliments: "Complimenten",
    courage: "Moed",
    diplomas: titleCase(finalCredentials),
    makeup: titleCase(makeUpActivities),
    skills: "Vaardigheden",
    specials: "Bijzonder",
    stages: `${titleCase(stage)}s`,
    start: "Start",
    technique: "Techniek"
