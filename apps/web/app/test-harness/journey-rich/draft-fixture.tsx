...
function Editor() {
  const [text,setText]=useState(''),[fail,setFail]=useState(true),[status,setStatus]=useState('Leeg'),[ready,setReady]=useState(false);
  const latest=useRef(text),stored=useRef(''),failure=useRef(fail);latest.current=text;failure.current=fail;
  useEffect(()=>{const unregister=registerDraftWriter(async()=>{
  ...
  },()=>latest.current!==stored.current);setReady(true);return unregister;},[]);
  // SSR must not accept fixture input before its simulated persistence guard is mounted.
  return <div><label>Fictief concept<input disabled={!ready} value={text} onChange={event=>setText(event.target.value)}/></label><label><input type="checkbox" disabled={!ready} checked={fail} onChange={event=>setFail(event.target.checked)}/>Simuleer opslagfout</label><p role="status">{status}</p></div>;
}
