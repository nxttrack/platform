...
export default async function ThemeImportPage({ searchParams }: { searchParams: Promise<{ import?: string }> }) {
  ...
  const recent = await createAdminClient().from("portal_theme_import").select("id, source_name, status, created_at").order("created_at", { ascending: false }).limit(40);
  if (recent.error) throw new Error("Importhistorie niet beschikbaar");
  return <div className="mx-auto max-w-5xl space-y-5"><Link href="/platform/themes" className="text-sm font-bold text-primary">← Themabibliotheek</Link><h1 className="text-3xl font-bold">Importeer een nieuwe zwemwereld</h1><p>Een pakket wordt eerst geanalyseerd en als concept bewaard. Publiceren en toewijzen zijn afzonderlijke stappen.</p><ThemePackageImport key={`${initial?.id}:${initial?.analysis.kind}`} initialResult={initial} /><details className="rounded-xl border p-4"><summary>Opgeslagen imports hervatten</summary><ul className="mt-3 grid gap-2">{(recent.data ?? []).map((row) => <li key={row.id}>{["analyzed", "draft"].includes(row.status) ? <Link className="underline" href={`/platform/themes/import?import=${row.id}`}>{row.source_name}</Link> : <span>{row.source_name}</span>} · {row.status} · {new Date(row.created_at).toLocaleString("nl-NL")}</li>)}</ul></details></div>;
}
