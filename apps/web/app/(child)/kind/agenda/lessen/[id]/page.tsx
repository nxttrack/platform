...
export default async function ChildLessonPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="child-page child-lesson-detail" data-child-route-state="lesson-detail">
    ...
    <div className="child-detail-grid">
      <section className="child-card child-lesson-detail-card"><CalendarDays /><small>Lesinformatie</small><strong>{formatDate(lesson.startsAt, data.tenant.timeZone)}</strong>{lesson.trainerFirstName ? <p><UserRound /> {terminology.instructor} {lesson.trainerFirstName}</p> : null}</section>
...
function formatDate(value: string, timeZone: string) { return formatChildLessonDate(value, { dateStyle: "full", timeStyle: "short" }, timeZone); }
