...
export async function generateMetadata(): Promise<Metadata> {
  const page = (await getPublicTenantSiteData())?.pages.news;
...
function Unavailable() {
  return <main className="grid min-h-[55vh] place-items-center px-4"><p className="rounded-xl border border-dashed border-border bg-card p-6 text-sm text-muted-foreground">Deze nieuwspagina is momenteel niet gepubliceerd.</p></main>;
}
