...
export default async function LegacyAutomationRulesPage({ searchParams }: { searchParams?: Promise<Record<string, string | string[] | undefined>> }) {
  const context = await requirePrivateShellContext("/admin/automatisering/regels");
  ...
  return <div className="space-y-6"><PageHeader kicker="Legacy regelbouwer" title="Eigen workflowregels" subtitle="Bestaande vrije regels blijven hier als configuratiearchief beschikbaar. Er is geen executor aan deze vrije regels gekoppeld; gebruik de recipegallery voor veilige, uitlegbare signalering." />
    <p className="rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-sm font-medium text-amber-950">Deze legacy-regels versturen niets en voeren geen mutaties uit. Actief betekent hier alleen dat de opgeslagen regel niet gepauzeerd is.</p>
