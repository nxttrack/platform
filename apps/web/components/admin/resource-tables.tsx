...
export function StudentsTable({ initialSearch, rows }: { initialSearch?: string; rows: StudentTableRow[] }) {
  const columns: ColumnDef<StudentTableRow, unknown>[] = [
    { id: "name", accessorFn: (row) => `${row.name} ${row.guardian}`, header: "Leerling", meta: { label: "Leerling" }, filterFn: dataTableTextFilter, cell: ({ row }) => <div><p className="font-semibold text-foreground">{row.original.name}</p><p className="text-xs text-muted-foreground">{row.original.guardian}</p>{row.original.isTest ? <StatusPill tone="info">Testdata</StatusPill> : null}</div> },
...
function LeadScoreDetails({ row }: { row: IntakeTableRow }) {
  return (
    <div className="grid gap-4">
      ...
      </div> : <p className="text-xs font-semibold text-muted-foreground">Testdata is uitlegbaar zichtbaar, maar kan geen echte CRM-taak of extern bericht maken.</p>}
    </div>
  );
}
