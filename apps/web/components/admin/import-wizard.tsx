  ...
  return (
    <form action={createImportJobAction} ref={formRef}>
      <Wizard completeLabel="Valideer import" onComplete={() => formRef.current?.requestSubmit()} steps={[
        ...
        { id: "upload", title: "CSV uploaden", description: "Maximaal 2 MB en 5.000 rijen. Komma en puntkomma worden automatisch herkend.", content: <label className="grid min-h-40 cursor-pointer place-items-center rounded-2xl border-2 border-dashed border-border bg-muted/30 p-5 text-center transition hover:border-primary/50"><FileSpreadsheet className="size-9 text-primary" /><span className="mt-2 font-bold text-foreground">Selecteer CSV-bestand</span><span className="text-sm text-muted-foreground">Voor verwerking volgt typecontrole, malwarecontrole en automatische dataclassificatie.</span><input accept=".csv,text/csv" className="mt-4 block max-w-full text-sm" name="file" required type="file" /></label> },
        { id: "validate", title: "Preview starten", description: "Na upload opent de werkbank voor veldmapping, referentiechecks, dry-run, apply en rollback.", content: <div className="grid gap-3 sm:grid-cols-2"><Info icon={ScanSearch} title="Automatische checks">Malware, classificatie, delimiter, headers, referenties, formaten en duplicaten.</Info><Info icon={ShieldCheck} title="Herstelbaar">Elke mutatie krijgt een manifest en auditlog voor veilige rollback.</Info></div> }
      ]} />
    </form>
  );
}
