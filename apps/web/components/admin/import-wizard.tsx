  ...
  return (
    <form action={createImportJobAction} ref={formRef}>
      <Wizard completeLabel="Valideer import" onComplete={() => formRef.current?.requestSubmit()} steps={[
        ...
        { id: "validate", title: "Preview & validatie", description: "Kolommen worden gemapt naar een veilige preview; lege en dubbele regels worden apart gemarkeerd.", content: <div className="grid gap-3 sm:grid-cols-2"><Info icon={ScanSearch} title="Automatische checks">Delimiter, headers, lege regels en exacte duplicaten.</Info><Info icon={ShieldCheck} title="Herstelbaar">Geen operationele data wordt zonder aparte goedkeuring gewijzigd.</Info></div> }
      ]} />
    </form>
  );
}
