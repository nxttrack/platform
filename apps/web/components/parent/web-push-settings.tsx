...
export function WebPushSettings({ activities, configured, enabled: initiallyEnabled, organization, preferences: initialPreferences, publicKey }: Props) {
  ...
  return (
    <div className="grid gap-5 lg:grid-cols-2">
      ...
      <div className="space-y-3">
        {!configured ? <p className="rounded-xl border border-warning/25 bg-warning/10 p-4 text-sm font-semibold text-warning-foreground">De {organization} heeft web-push nog niet technisch geactiveerd.</p> : null}
        {enabled ? <>
          <Toggle checked={preferences.lessonChanges} label={`Wijzigingen en annuleringen van ${activities}`} onChange={(value) => setPreferences((current) => ({ ...current, lessonChanges: value }))} />
          ...
          <Toggle checked={preferences.reminders} label={`Behulpzame herinneringen voor ${activities}`} onChange={(value) => setPreferences((current) => ({ ...current, reminders: value }))} />
