...
export function FamilyCommandCenter({ children, contextParticipantId }: { children: FamilyChild[]; contextParticipantId?: string | null }) {
      ...
      <div className="grid gap-0 lg:grid-cols-[0.8fr_1.2fr]">
        <div className="border-b border-border p-4 sm:p-5 lg:border-b-0 lg:border-r">
          ...
          <div className="mt-5 rounded-2xl bg-primary/5 p-4">
            ...
            <Link className="mt-4 inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-primary px-4 text-sm font-bold text-primary-foreground sm:w-auto" href={withParticipant("/portaal/planning", contextParticipantId)}>Bekijk planning<ChevronRight className="size-4" /></Link>
          </div>
          <div className="mt-5">
...
function withParticipant(href: string, participantId?: string | null) {
  return participantId ? `${href}?kind=${encodeURIComponent(participantId)}` : href;
}
