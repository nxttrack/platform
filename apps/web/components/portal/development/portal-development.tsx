...
export function PortalDevelopment({ model, chapters, badges, contextKey, participantId, audience = "parent", canAsk = false, itemContent }: {
  ...
  const selectedId = params.get("onderdeel") ?? params.get("focus"), selected = !params.get("hoofdstuk") && (audience === "parent" || params.get("detail") === "1") ? model.items.find((item) => item.id === selectedId || item.curriculumItemId === selectedId) ?? null : null;
