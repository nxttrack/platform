...
export function DataTable<TData>({
  ...
  async function runBulkAction(action: DataTableBulkAction<TData>) {
    if (action.confirm && !window.confirm(action.confirm)) return;
    setBulkBusy(action.label);
    try {
      const result = await action.onSelect(selectedRows);
      setRowSelection({});
      if (result?.undo) {
