...
test("microcopy en notificatiebadges houden op het ouderdashboard WCAG AA-contrast", async () => {
  ...
  assert.match(shell, /mobileBottomNav \? "portal-parent-brand-title" : "text-muted-foreground"/);
  assert.match(styles, /\.portal-parent-brand-title\s*\{[^}]*color:\s*var\(--portal-text\)/s);
...
test("authenticated parent smoke volgt de actuele rich Journey renderer en accepteert iedere catalogustheme", async () => {
  const { readFile } = await import("node:fs/promises");
  ...
  for (const theme of PORTAL_VISUAL_THEMES) assert.match(source, new RegExp(theme));
});
...
test("canonieke releases blijven main-only en fixtures behouden beheeridentiteiten", async () => {
  const { readFile } = await import("node:fs/promises");
  ...
  assert.match(workflow, /release:assert-source/);
  assert.doesNotMatch(workflow, /staging_preview_sha|STAGING_PREVIEW_SHA|codex\/parent-child-portals-v1/);
  assert.match(workflow, /PHASE16_RESET_E2E_PASSWORDS:[^\n]+false/);
  assert.match(workflow, /PHASE16_PRESERVE_ADMIN_IDENTITIES:[^\n]+true/);
  assert.match(workflow, /Prepare Sprint 4 browser-mutation fixture/);
